...
-- Idempotent: FK to bookings is optional

SET @bookings_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'bookings');

-- Create invoices table without FK first, then add FK if bookings exists
...
-- Add FK to bookings only if it exists
SET @fk_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'invoices' AND CONSTRAINT_NAME = 'fk_invoices_reservation');
SET @add_fk := IF(@bookings_exists = 1 AND @fk_exists = 0, 'ALTER TABLE invoices ADD CONSTRAINT fk_invoices_reservation FOREIGN KEY (reservation_id) REFERENCES bookings(id)', 'SELECT 1');
PREPARE stmt FROM @add_fk; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Add avatar column to restaurants if it doesn't exist
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'restaurants' AND COLUMN_NAME = 'avatar');
SET @ddl := IF(@col_exists = 0, 'ALTER TABLE restaurants ADD COLUMN avatar VARCHAR(1024) NULL AFTER name', 'SELECT 1');
