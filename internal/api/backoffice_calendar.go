...
// Mirrors legacy /api/get_calendar_data.php but scoped to the active backoffice restaurant.
func (s *Server) handleBOCalendarMonth(w http.ResponseWriter, r *http.Request) {
	...
	// Reuse the same in-memory cache used by the public endpoint.
	...
	if defaults, err := s.loadReservationDefaults(r.Context(), restaurantID); err == nil && defaults.DailyLimit > 0 {
		defaultLimit = defaults.DailyLimit
	}
