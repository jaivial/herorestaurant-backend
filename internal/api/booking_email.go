...
func smtpSend(ctx context.Context, host string, port int, username, password, fromName, fromAddr, to, subject, htmlBody string) error {
...
func buildBookingEmailHTML(brandName string, logoURL string, booking map[string]any, bookingID int64, baseURL string) string {
...
<p style="margin-bottom:20px;">Si necesita modificar o cancelar su reserva, por favor contáctenos:</p>
<ul style="margin-bottom:30px;padding-left:20px;">
<li>Teléfono: <a href="tel:+34638857294" style="color:#097969;text-decoration:none;">638 85 72 94</a></li>
<li>Email: <a href="mailto:reservas@alqueriavillacarmen.com" style="color:#097969;text-decoration:none;">reservas@alqueriavillacarmen.com</a></li>
</ul>
...
func sendBookingConfirmationEmails(ctx context.Context, s *Server, restaurantID int, booking map[string]any, bookingID int64) (customerSent bool, restaurantSent bool, err error) {
	...
	if fromAddr == "" {
		return false, false, fmt.Errorf("SMTP no configurado (falta email de origen)")
	}
	...
	html := buildBookingEmailHTML(brandName, logoURL, booking, bookingID, baseURL)

	// Send to customer.
	customerEmail := strings.TrimSpace(anyToString(booking["contact_email"]))
	if customerEmail != "" && customerEmail != fromAddr {
		if e := smtpSend(ctx, host, port, username, password, fromName, fromAddr, customerEmail, subject, html); e != nil {
	...
	if e := smtpSend(ctx, host, port, username, password, fromName, fromAddr, fromAddr, subject, html); e != nil {
