...
func (s *Server) publicMenuMediaURL(ctx context.Context, raw string) string {
	...
	return s.bunnyPullURL(ctx, value)
}
...
func (s *Server) loadPublicSliderImages(ctx context.Context, restaurantID int, menuID int64) (string, []string) {
	...
	for rows.Next() {
		...
		if url := s.publicMenuMediaURL(ctx, path); url != "" {
	...
	if isHomePage {
		...
		for rows.Next() {
			...
			menus = append(menus, publicMenuItemHome{
				...
				MenuPreviewImageURL:  s.publicMenuMediaURL(r.Context(), menuPreviewPathRaw.String),
				SpecialMenuImageURL:  s.publicMenuMediaURL(r.Context(), specialImageURLRaw.String),
			})
		}
	...
	if len(menuIDs) > 0 {
		...
		for dishRows.Next() {
			...
			dish := publicMenuDish{
				...
				FotoURL:           s.publicMenuMediaURL(r.Context(), fotoPath.String),
	...
	if menuType == "special" {
		...
		httpx.WriteJSON(w, http.StatusOK, map[string]any{
			...
			"menu": publicMenuItemSpecial{
				...
				MenuSubtitle:        anySliceToStringList(decodeJSONOrFallback(menuSubtitleRaw.String, []any{})),
				Comments:            anySliceToStringList(decodeJSONOrFallback(commentsRaw.String, []any{})),
				SpecialMenuImageURL: s.publicMenuMediaURL(r.Context(), specialImageURL.String),
			},
		})
		return
	}
