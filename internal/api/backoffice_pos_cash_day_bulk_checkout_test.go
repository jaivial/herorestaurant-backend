...
// A visit with several tickets closes only once its last ticket is paid, and a
// zero-value ticket is skipped (no pos_payments row) — so it stays OPEN and keeps
// the visit open. This guards the extraction of the openTickets==0 branch.
func TestPOSCashDayBulkCheckoutClosesMultiTicketVisitAndSkipsZero(t *testing.T) {
	db, s := posCashDayTestDB(t)
	recorder := httptest.NewRecorder()
	s.handleBOPOSCashDayOpen(recorder, posCashDayRequest(http.MethodPost, "/admin/pos/cash-days", `{"date":"2024-03-08","openingCashCents":0}`, nil))
	if recorder.Code != http.StatusCreated {
		t.Fatalf("open cash day: got %d %s", recorder.Code, recorder.Body.String())
	}
	day, _ := decodeCashDayBody(t, recorder)["cashDay"].(map[string]any)
	...
	statements := []string{
		...
		`INSERT INTO pos_visits(id,restaurant_id,cash_day_id,channel,table_id,service_date,service_type,covers,status,opened_by,open_idempotency_key,opened_at) VALUES(142,1,` + strconv.FormatInt(dayID, 10) + `,'DINE_IN',7,'2024-03-08','DINNER',3,'OPEN',7,'bv142','2024-03-08 21:00:00')`,
		`INSERT INTO pos_tickets(id,restaurant_id,visit_id,shift_id,ticket_number,creation_idempotency_key,subtotal_gross_cents,total_gross_cents,status,opened_by) VALUES(152,1,142,` + strconv.FormatInt(shiftID, 10) + `,'TPV-152','t152',100,100,'OPEN',7)`,
		`INSERT INTO pos_tickets(id,restaurant_id,visit_id,shift_id,ticket_number,creation_idempotency_key,subtotal_gross_cents,total_gross_cents,status,opened_by) VALUES(153,1,142,` + strconv.FormatInt(shiftID, 10) + `,'TPV-153','t153',100,100,'OPEN',7)`,
		`INSERT INTO pos_tickets(id,restaurant_id,visit_id,shift_id,ticket_number,creation_idempotency_key,subtotal_gross_cents,total_gross_cents,status,opened_by) VALUES(154,1,142,` + strconv.FormatInt(shiftID, 10) + `,'TPV-154','t154',0,0,'OPEN',7)`,
		`INSERT INTO pos_ticket_lines(id,restaurant_id,ticket_id,pos_product_id,product_name_snapshot,quantity,unit_price_gross_cents,vat_rate_snapshot,discount_cents,line_total_gross_cents,status,idempotency_key,created_by) VALUES(170,1,152,30,'Cerveza',1,100,0,0,100,'ACTIVE','l170',7)`,
		`INSERT INTO pos_ticket_lines(id,restaurant_id,ticket_id,pos_product_id,product_name_snapshot,quantity,unit_price_gross_cents,vat_rate_snapshot,discount_cents,line_total_gross_cents,status,idempotency_key,created_by) VALUES(171,1,153,30,'Cerveza',1,100,0,0,100,'ACTIVE','l171',7)`,
	}
	for _, statement := range statements {
		if _, err := db.Exec(statement); err != nil {
			t.Fatalf("%s: %v", statement, err)
	...
	recorder = httptest.NewRecorder()
	s.handleBOPOSCashDayBulkCheckout(recorder, posCashDayRequest(http.MethodPost, "/admin/pos/cash-days/2024-03-08/bulk-checkout", `{"paymentMethod":"CASH","idempotencyKey":"bulk-multi"}`, map[string]string{"date": "2024-03-08"}))
	if recorder.Code != http.StatusOK {
		t.Fatalf("bulk checkout: got %d %s", recorder.Code, recorder.Body.String())
	}
	body := decodeCashDayBody(t, recorder)
	...
	if body["skippedTickets"].(float64) != 1 {
		t.Fatalf("expected 1 skipped ticket, got %v", body["skippedTickets"])
	}
	// The zero-value ticket stays OPEN, so the visit cannot close yet.
	if body["closedVisits"].(float64) != 0 {
		t.Fatalf("expected 0 closed visits (zero ticket still open), got %v", body["closedVisits"])
	}
	for _, id := range []int64{152, 153} {
		var status string
		if err := db.QueryRow(`SELECT status FROM pos_tickets WHERE id=?`, id).Scan(&status); err != nil {
			t.Fatal(err)
		}
	...
	var zeroStatus, visitStatus string
	if err := db.QueryRow(`SELECT status FROM pos_tickets WHERE id=154`).Scan(&zeroStatus); err != nil {
		t.Fatal(err)
	}
	if zeroStatus != "OPEN" {
		t.Fatalf("zero-value ticket 154 expected OPEN, got %s", zeroStatus)
	}
	if err := db.QueryRow(`SELECT status FROM pos_visits WHERE id=142`).Scan(&visitStatus); err != nil {
		t.Fatal(err)
	}
	if visitStatus != "OPEN" {
		t.Fatalf("visit 142 expected OPEN while a ticket is still open, got %s", visitStatus)
	}
}

// Non-cash methods are rejected at the bulk boundary: card payments need a
// per-ticket terminal reference that a date-wide sweep cannot supply.
func TestPOSCashDayBulkCheckoutRejectsNonCash(t *testing.T) {
	_, s := posCashDayTestDB(t)

	recorder := httptest.NewRecorder()
	s.handleBOPOSCashDayBulkCheckout(recorder, posCashDayRequest(http.MethodPost, "/admin/pos/cash-days/2024-03-07/bulk-checkout", `{"paymentMethod":"CARD","idempotencyKey":"bulk-card"}`, map[string]string{"date": "2024-03-07"}))
	if recorder.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for CARD bulk, got %d %s", recorder.Code, recorder.Body.String())
	}
}
