...
func renderBotSystemPrompt(d botPromptData) string {
	...
	b.WriteString("- Extras de la reserva (añadidos como café incluido o bebida ilimitada): usa `list_booking_extras` para conocer los disponibles. Aparecen en `get_bookings` dentro del campo `extras` de cada reserva. Solo aplican a reservas SIN menú de grupo.\n")
