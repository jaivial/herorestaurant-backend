...
- `key` (string): p. ej. `reservasDisplayMode`
- `value` (string): p. ej. `tabla` | `grid`

Sólo se aceptan claves/valores en la lista blanca (`reservasDisplayMode` → `tabla` | `grid`);
los valores se normalizan a minúsculas.

Response:

- `{ success: true, preferences: { ... } }` — preferencias actualizadas del restaurante activo.
- `{ success: false, message }` si la clave/valor no son válidos o no hay restaurante activo.
